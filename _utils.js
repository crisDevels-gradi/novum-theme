ReCharge.Novum.Utils = {
    ...
    /** Gets the formatted payment method brand. Only returns for credit cards*/
    getFormattedBrand: (paymentMethod) => paymentMethod.payment_details?.brand?.toLowerCase().replace('_', ' ') || '',
    ...
    bulkCancelAddonProducts: async function(evt, url) {
        ...
        if(window.analytics && url.includes("cancel")) {
            window.analytics.track("Customer Portal Modal - Submit - Cancel", {theme: "novum"})
        }
        ...
        try {
            ...
            if(window.heap && url.includes("cancel")){
                window.heap.track('Cancellation Complete');
            }
            if(window.analytics && url.includes("cancel")) {
                window.analytics.track("Cancellation Complete", {theme: "novum"})
            }
            ...
            console.error(error);
            if(window.analytics && url.includes("cancel")) {
                window.analytics.track("Cancellation Error", {theme: "novum"})
            }
            ReCharge.Toast.addToast(`{{ 'cp_toast_error' | t }}`, `{{ 'cp_unable_to_perform_update' | t }}`);
            ReCharge.Forms.toggleSubmitButton(evt.target);
        } finally {
    ...
    translateOrderIntervalUnit: function(orderIntervalUnit, frequency) {
        const unit = orderIntervalUnit ? orderIntervalUnit.toLowerCase(): "";
