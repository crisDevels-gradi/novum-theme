function configureStore() {
    ...
    if (settingsStore.customer_portal.edit_shipping_address) {
        if (editAddressBtn) {
            ...
            document
                ...
                .forEach(address => {
                    ReCharge.Novum.Utils.addAccessibleClickListener(address, renderAddressDetailsHandler)
                });
        }
    } else {
        if (editAddressBtn) {
    ...
    if (settingsStore.customer_portal.subscription.edit_scheduled_date) {
        if (editNextChargeDateBtn) {
            ...
            ReCharge.Novum.Utils.addAccessibleClickListener(nextDeliveryContainer, editNextShipment);
        }
    } else {
        if (editNextChargeDateBtn) {
            ReCharge.Novum.Helpers.showElement(editNextChargeDateBtn);
    ...
    if (settingsStore.customer_portal.subscription.edit_order_frequency !== "Prohibited") {
        if (frequencyBtn) {
            ...
            ReCharge.Novum.Utils.addAccessibleClickListener(frequencyContainer, editScheduleHandler);
        }
    } else {
        if (frequencyBtn) {
            ReCharge.Novum.Helpers.showElement(frequencyBtn);
...
(function() {
    ...
    ReCharge.Novum.toggleSidebar = function(originatingElement) {
        ...
        if(sidebarElement.classList.contains('visible')) {
            if(window.analytics) window.analytics.track("Customer Portal Modal - Click - Close", {theme: "novum"}) 
        }
