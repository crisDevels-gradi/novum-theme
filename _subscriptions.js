...
function renderAddProductDetails(product) {
    ...
    const isRcpmEnabled = storeSettings.customer_portal.view_recharge_payment_methods;
    ...
    function renderAddressOption(address) {
        if (isRcpmEnabled) {
            ...
            const { brand, last4 } = paymentMethod.payment_details;

            return `
                <option value="${address.id}">
                    ${address.address1}&nbsp; | &nbsp;${ReCharge.Utils.capitalize(brand)} ••••${last4}
                </option>
            `;
        };
