...
(function () {
  ...
  function getDynamicPaymentMethodContent(paymentMethod) {
    ...
    return {
      ...
      paymentType: brand ? translations.paymentMethod.last4.replace('{brand}', capitalize(brand)).replace('{last4}', paymentDetails.last4) : capitalize(paymentType.toLowerCase().replace('_', ' ')),
      cardSummary: `
  ...
  function renderPaymentMethods() {
              ...
              <div class="billing-address-container">
                ...
                ${render(allowEdit && `
                  <button class="edit-billing-address rc-btn rc-btn--link" type="button">
                    <span class="rc-btn--icon">{% include '_edit-icon.svg' %}</span>
                    ${translations.paymentMethods.editAddressBtn}
                  </button>
                  `)}
              </div>
            </div>
    ...
    if (!document.querySelector('.payment-method[data-payment-method-id]')) {
      paymentMethodCardsEl.innerHTML = `<p class="empty">${translations.paymentMethods.noResults}</p>`;
    } else if (emptyEl) {
  ...
  function onEditPaymentMethod(e) {
    ...
    const isPaypal = selectedPaymentMethod?.payment_type === 'PAYPAL';
    ...
    addCardWindowListener(async () => {
      ReCharge.Toast.addToast(`{{ 'cp_toast_success' | t }}`, translations.paymentMethods.edit.success);
      ReCharge.Drawer.close();
      try {
        const { data: { payment_method } } = await axios.get(ReCharge.Endpoints.payment_methods(paymentMethodId));
        findAndReplacePaymentMethod(payment_method);
        renderPaymentMethods();
      } catch (e) {
