...
function cancelSubscriptionFlow(event) {
    ...
    if(ReCharge.Novum.backBtn.style.visibility === "hidden") {
        if(window.analytics) window.analytics.track("Customer Portal - Click - Cancel Subscription", {theme: "novum"}) 
    } else {
        if(window.analytics) window.analytics.track("Customer Portal Modal - Click - Back", {theme: "novum"}) 
    }
    ...
    document.querySelectorAll('input[name=retention_strategy_id]').forEach((element)=> {
        element.addEventListener("click", logStrategySelection)
    }) 
    !event.target.closest('.js-back-btn') && ReCharge.Novum.toggleSidebar(event.currentTarget);
}

function logStrategySelection(ev) {
    if(window.analytics) {
        window.analytics.track("Customer Portal Modal - Click - Cancellation Reason", {reason: ev.srcElement.id, theme: "novum"})
    }
}
...
function renderRetentionStrategiesHandler(event) {
    ...
    try {
        ...
        if(window.analytics) {
            window.analytics.track("Customer Portal Modal - Click - Next Empty", {theme: "novum"})
        }
        console.log(error)
        return
    }
    ...
    if(window.analytics) window.analytics.track("Customer Portal Modal - Click - Next", {theme: "novum", incentive_type: incentive_type});
...
function skipShipmentHandler(event) {
    ...
    ReCharge.Novum.sidebarContent.innerHTML = `
        ...
        <button
            type="button"
            class="rc_btn text_uppercase title-bold"
